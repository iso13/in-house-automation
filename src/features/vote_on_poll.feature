Feature: Vote On Poll
  ...
  Scenario: Successfully vote on poll
    When I enter valid information to vote on poll
    And I submit the form
    Then I should see a confirmation message
    And the poll should be saved

  Scenario: vote on poll using maximum input values
  ...
  Scenario: Attempt to vote on poll with missing required fields
    When I submit the form with incomplete information
    Then I should see validation messages for required fields

  Scenario: Attempt to vote on poll with invalid input types
    When I provide invalid data formats
    And I submit the form
    Then the system should reject the input
    And I should remain on the poll page

  Scenario: Navigate away and return to resume vote on poll
    When I start to vote on poll
