Feature: Remove Post
  ...
  Scenario: Successfully remove post
    When I enter valid information to remove post
    And I submit the form
    Then I should see a confirmation message
    And the post should be saved

  Scenario: remove post using maximum input values
  ...
  Scenario: Attempt to remove post with missing required fields
    When I submit the form with incomplete information
    Then I should see validation messages for required fields

  Scenario: Attempt to remove post with invalid input types
    When I provide invalid data formats
    And I submit the form
    Then the system should reject the input
    And I should remain on the post page

  Scenario: Navigate away and return to resume remove post
    When I start to remove post
