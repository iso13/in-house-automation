Feature: Add Post
  ...
  Scenario: Successfully add post
    When I enter valid information to add post
    And I submit the form
    Then I should see a confirmation message
    And the post should be saved

  Scenario: add post using maximum input values
  ...
  Scenario: Attempt to add post with missing required fields
    When I submit the form with incomplete information
    Then I should see validation messages for required fields

  Scenario: Attempt to add post with invalid input types
    When I provide invalid data formats
    And I submit the form
    Then the system should reject the input
    And I should remain on the post page

  Scenario: Navigate away and return to resume add post
    When I start to add post
